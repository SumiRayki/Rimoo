...
abstract class DataBaseKT : RoomDatabase() {
    ...
    companion object {
        ...
        private fun initPhrasesDb() {  //初始化常用语数据数据
            ...
            if (runCatching { instance.phraseDao().queryByContent("情绪价值拉满") }.getOrNull() == null) {
                val bundledPhrases = BundledPhraseLoader.load()
                if (bundledPhrases.isNotEmpty()) {
                    instance.phraseDao().insertAll(bundledPhrases)
                }
            }
