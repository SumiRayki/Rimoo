...
class EnvironmentSingleton private constructor() {
    ...
    private var keyboardWidthRatio = 1f
    ...
    fun initData() {
        ...
        val baseInputWidth = screenWidthVertical - holderWidth
        inputAreaWidth = if (isLandscape && !keyboardModeFloat) mScreenWidth else baseInputWidth
        ...
        keyboardWidthRatio = when {
            keyboardModeFloat && isLandscape -> AppPrefs.getInstance().internal.keyboardWidthRatioLandscapeFloat.getValue()
            keyboardModeFloat -> AppPrefs.getInstance().internal.keyboardWidthRatioFloat.getValue()
            isLandscape -> AppPrefs.getInstance().internal.keyboardWidthRatioLandscape.getValue()
            else -> AppPrefs.getInstance().internal.keyboardWidthRatio.getValue()
        }

        val minKeyboardWidth = min(DevicesUtils.dip2px(220), inputAreaWidth)
        skbWidth = (baseInputWidth * keyboardWidthRatio).toInt().coerceIn(minKeyboardWidth, inputAreaWidth)
        skbHeight = (screenHeightVertical * keyboardHeightRatio).toInt()

        heightForcomposing = (
            screenHeightVertical * candidatesHeightRatio *
                AppPrefs.getInstance().keyboardSetting.candidateTextSize.getValue() / 100f
            ).toInt()
    ...
    var keyBoardHeightRatio: Float
        ...
        set(keyBoardHeightRatio) {
            ...
            if (isLandscape) {
                AppPrefs.getInstance().internal.keyboardHeightRatioLandscape.setValue(keyBoardHeightRatio)
            } else {
                AppPrefs.getInstance().internal.keyboardHeightRatio.setValue(keyBoardHeightRatio)
            }
        }

    var keyBoardWidthRatio: Float
        get() = keyboardWidthRatio
        set(keyBoardWidthRatio) {
            keyboardWidthRatio = keyBoardWidthRatio
            when {
                keyboardModeFloat && isLandscape -> AppPrefs.getInstance().internal.keyboardWidthRatioLandscapeFloat.setValue(keyBoardWidthRatio)
                keyboardModeFloat -> AppPrefs.getInstance().internal.keyboardWidthRatioFloat.setValue(keyBoardWidthRatio)
                isLandscape -> AppPrefs.getInstance().internal.keyboardWidthRatioLandscape.setValue(keyBoardWidthRatio)
                else -> AppPrefs.getInstance().internal.keyboardWidthRatio.setValue(keyBoardWidthRatio)
            }
        }
    ...
    var keyboardModeFloat: Boolean
        ...
        set(isFloatMode) {
            if (isLandscape) {
                AppPrefs.getInstance().internal.keyboardModeFloatLandscape.setValue(isFloatMode)
            } else {
                AppPrefs.getInstance().internal.keyboardModeFloat.setValue(isFloatMode)
            }
        }

    val skbAreaHeight: Int
        get() = instance.inputAreaHeight + if (!instance.keyboardModeFloat) {
            AppPrefs.getInstance().internal.keyboardBottomPadding.getValue() +
                instance.systemNavbarWindowsBottom +
                if (AppPrefs.getInstance().internal.fullDisplayKeyboardEnable.getValue()) {
                    instance.heightForFullDisplayBar
                } else {
                    0
                }
        } else {
            instance.heightForKeyboardMove
        }

    val leftMarginWidth: Int
        get() = if (!instance.keyboardModeFloat) {
            val rightGap = AppPrefs.getInstance().internal.keyboardRightPadding.getValue()
            (instance.inputAreaWidth - instance.skbWidth - rightGap).coerceAtLeast(0)
        } else {
            0
        }
