    ...
    private fun updateCandidatesOrCommitText(): String? {
        ...
        showCandidates = when {
            compositionText.isNotBlank() -> {
                val phrase = CustomEngine.processPhrase(compositionText)
                if(InputModeSwitcherManager.isEnglish && StringUtils.isLetter(compositionText) &&
                    candidates.firstOrNull()?.text?.equals(compositionText, ignoreCase = true) == false ){
        ...
        if (compositionText.isNotBlank()) {
            customPhraseSize = CustomEngine.processPhrase(compositionText).size
            if (InputModeSwitcherManager.isEnglish &&
                StringUtils.isLetter(compositionText) &&
                candidates.firstOrNull()?.text?.equals(compositionText, ignoreCase = true) == false
            ) {
                customPhraseSize += 1
            }
        }
        if (customPhraseSize > 0 && showCandidates.size > customPhraseSize) {
            showCandidates = showCandidates.drop(customPhraseSize) + showCandidates.take(customPhraseSize)
            customPhraseSize = 0
        }
