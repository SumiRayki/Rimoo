...
object CustomEngine {
    ...
    fun processPhrase(text: String):MutableList<String> {
        ...
        if(chinesePredictionDate) {
            ...
            val phraseContents = linkedSetOf<String>()
            if (normalizedText.length >= 6) {
                phraseContents.addAll(
                    DataBaseKT.instance.phraseDao().queryExact(normalizedText).map { it.content }
                )
            }
            phrases.addAll(phraseContents)
            val suffixesDate = setOf("rq", "riqi", "7474", "77")
            if (suffixesDate.any { it == normalizedText }) {
                phrases.addAll(TimeUtils.getData())
            }
            val suffixesTime = setOf("sj", "shijian", "75", "7445426")
            if (suffixesTime.any { it == normalizedText }) {
