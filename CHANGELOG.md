...
**⚠️如您由0.16.0之前的版本升級，由於參數變化，安裝Rimoo前請保存好文件資料，於安裝後重啓或註銷 Windows，否則正在使用Rimoo的應用可能會崩潰。**
...
**⚠️由於參數變化，安裝Rimoo前請保存好文件資料，於安裝後重啓或註銷 Windows，否則正在使用Rimoo的應用可能會崩潰。**
...
* IME改爲可選項，默認不安裝
* 棄用 `weaselt*.dll`，增加註冊香港、澳門、新加坡區域語言配置（默認未啓用，需在控制面板/設置中手工添加）；支持簡繁體Rimoo同時使能
* 棄用 `weaselt*.ime`
* 移除 `pyweasel`
* 候選窗口 UI 內存優化
* 改善候選窗口 UI 繪製性能
...
* 支持命令行設置Rimoo `ascii_mode` 狀態，`WeaselServer.exe /ascii`，`WeaselServer.exe /nascii`
...
**⚠️安裝Rimoo前請保存好文件資料，於安裝後重啓 Windows ，否則正在使用Rimoo的應用將會崩潰。**
**⚠️此版本的Rimoo需要使用 Windows 8.1 或更高版本的操作系統。**
...
* 修復部分內存訪問問題
* 修復操作系統 / WinGet 無法識別Rimoo版本號的問題
* 修復 composition 模式下光標位置不正常的問題
* 修復 Word 中Rimoo工作不正常的問題
* 若干開發環境配置問題修復
