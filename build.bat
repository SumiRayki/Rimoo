...
if not defined WEASEL_BUNDLED_RECIPES set WEASEL_BUNDLED_RECIPES=:preset pinyin-simp double-pinyin wubi
...
call :render_template weasel.props %WEASEL_PROJECT_PROPERTIES%
...
rem -------------------------------------------------------------------------
rem %1 : target file
rem %2... : environment variable names to substitute into %1.template
:render_template
  cscript.exe //nologo render.js %*
  if not errorlevel 1 exit /b 0

  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$target = 'weasel.props'; $template = [System.IO.File]::ReadAllText($target + '.template', [System.Text.Encoding]::UTF8); $map = @{ 'BOOST_ROOT' = $env:BOOST_ROOT; 'PLATFORM_TOOLSET' = $env:PLATFORM_TOOLSET; 'VERSION_MAJOR' = $env:VERSION_MAJOR; 'VERSION_MINOR' = $env:VERSION_MINOR; 'VERSION_PATCH' = $env:VERSION_PATCH; 'PRODUCT_VERSION' = $env:PRODUCT_VERSION; 'FILE_VERSION' = $env:FILE_VERSION }; foreach ($entry in $map.GetEnumerator()) { $value = if ($null -eq $entry.Value) { '' } else { [string]$entry.Value }; $template = $template.Replace('$' + $entry.Key, $value) }; [System.IO.File]::WriteAllText($target, $template, (New-Object System.Text.UTF8Encoding($false)))"
  exit /b %errorlevel%
...
:build_data
  ...
  if errorlevel 1 goto error
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$path = 'output/data/default.yaml'; $content = [System.IO.File]::ReadAllText($path, [System.Text.Encoding]::UTF8); $old = @'`nschema_list:`n  - schema: luna_pinyin`n  - schema: luna_pinyin_simp`n  - schema: luna_pinyin_fluency`n  - schema: bopomofo`n  - schema: bopomofo_tw`n  - schema: cangjie5`n  - schema: stroke`n  - schema: terra_pinyin`n'@; $new = @'`nschema_list:`n  - schema: luna_pinyin_simp`n  - schema: luna_pinyin`n  - schema: luna_pinyin_fluency`n  - schema: double_pinyin_flypy`n  - schema: double_pinyin_mspy`n  - schema: wubi86`n  - schema: bopomofo`n  - schema: bopomofo_tw`n  - schema: cangjie5`n  - schema: stroke`n  - schema: terra_pinyin`n'@; if ($content.Contains($old)) { $content = $content.Replace($old, $new) }; [System.IO.File]::WriteAllText($path, $content, (New-Object System.Text.UTF8Encoding($false)))"
  if errorlevel 1 goto error
  exit /b

rem ---------------------------------------------------------------------------
:build_opencc_data
  set OPENCC_DATA_DIR=%WEASEL_ROOT%\librime\share\opencc
  if not exist %OPENCC_DATA_DIR%\TSCharacters.ocd2 (
    if exist %WEASEL_ROOT%\librime\rime-de4700e-Windows-msvc-x86\share\opencc\TSCharacters.ocd2 (
      set OPENCC_DATA_DIR=%WEASEL_ROOT%\librime\rime-de4700e-Windows-msvc-x86\share\opencc
    ) else if exist %WEASEL_ROOT%\librime\rime-de4700e-Windows-msvc-x64\share\opencc\TSCharacters.ocd2 (
      set OPENCC_DATA_DIR=%WEASEL_ROOT%\librime\rime-de4700e-Windows-msvc-x64\share\opencc
    ) else (
      cd %WEASEL_ROOT%\librime
  ...
  copy %OPENCC_DATA_DIR%\*.* output\data\opencc\
  if errorlevel 1 goto error
  exit /b
